...
posts = (
    (1, "Barbus fasciolatus", "Afrikansk bandbarb", "Temperatur: 20 - 26 °C  Längd: 6 cm  Ursprunglig världsdel: Afrika  pH: 6-7  Minsta akvarie storlek: 80 cm & 100 liter  Svårighetsgrad: 3/5", 5, 1, time.strftime("%Y-%m-%d %H:%M:%S")),
    (2, "Pangio kuhlii", "Kuhlii-ål", "", 4, 1, time.strftime("%Y-%m-%d %H:%M:%S")),
    (1, "Paracheirodon innesi", "Neontetra", "", 10, 1, time.strftime("%Y-%m-%d %H:%M:%S")),
    (2, "Pethia padamya", "Odessabarb", "", 5, 1, time.strftime("%Y-%m-%d %H:%M:%S")),
    (1, "Thayeria boehlkei", "Vinkeltetra", "Temperatur: 23 - 28 °C  Längd: 6 cm  Ursprunglig världsdel: Sydamerika  pH: 6-7.5  Minsta akvarie storlek: 80 cm & 90 liter  Svårighetsgrad: 2/5", 5, 1, time.strftime("%Y-%m-%d %H:%M:%S")),
    (4, "Trigonostigma heteromorpha", "Kilfläcksrasbora", "", 3, 1, time.strftime("%Y-%m-%d %H:%M:%S")),
    (2, "Pterophyllum scalare", "Skalar", "Temperatur: 24 - 26 °C  Längd: 16 cm  Ursprunglig världsdel: Sydamerika  pH: 6-7.5  Minsta akvarie storlek: 90 cm & 200 liter  Svårighetsgrad: 2/5 ", 5, 1, time.strftime("%Y-%m-%d %H:%M:%S")),
    (3, "Trigonostigma heteromorpha", "Kilfläcksrasbora", "", 7, 1, time.strftime("%Y-%m-%d %H:%M:%S")),
)
...
with conn:
    ...
    cur.executemany("INSERT INTO posts (seller_id, scientific_name, plain_name, description, quantity, type, time_stamp_registration) VALUES(?, ?, ?, ?, ?, ?, ?)", posts)
